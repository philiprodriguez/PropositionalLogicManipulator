...
/**
 * Created by Philip James Rodriguez on 4/7/2016.
 * Applied courses: COT3100, COP3502
 *
 * This program can do a few different things. It
 * can convert infix logical expressions to postfix
 * logical expressions, and then can evaluate those
 * postfix logical expressions.
 *
 * This program generates full truth tables unlike
 * most online truth table generators, which only
 * generate the variable columns and the final
 * result column.
 */
public class Main {
    ...
    public static TruthTable postFixToTruthTable(String postfixLogicalExpression) throws Exception {
        ...
        for(int i = 0; i < postfixLogicalExpression.length(); i++) {
            ...
            else
            {
                ...
                /*
                We used to do this here to just fill our table:

                truthTable.addItemToColumn(miniOutput, false);
                truthTable.addItemToColumn(miniOutput, false);
                truthTable.addItemToColumn(miniOutput, false);
                truthTable.addItemToColumn(miniOutput, false);
                truthTable.addItemToColumn(miniOutput, false);
